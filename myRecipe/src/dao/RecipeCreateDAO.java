...
public class RecipeCreateDAO {
	public boolean CreateRecipe(RecipeEntity recipe){
		String checkSql = "SELECT dish FROM recipe WHERE dish = ? AND mail_id = ?";
		String registerSql = "INSERT INTO recipe(dish, ing_1, ing_2, ing_3, ing_4, ing_5, ing_6, ing_7, ing_8, ing_9, ing_10, cooking_time, mail_id) "
				+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
		int result;
		boolean ret = true;
		...
		try{
			...
			ps1.setString(2, recipe.getMail());
			ResultSet rs = ps1.executeQuery();

			if(recipe.getDish().equals("")){
				ret = false;
			}else if(rs.next()){
				recipe.setDupeFlg(false);
				ret = false;
			}else{
				...
				ps2.setString(13, recipe.getMail());
